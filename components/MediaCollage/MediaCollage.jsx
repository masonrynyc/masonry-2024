	...
	return (
		...
		>
			<div className="flex px-margin flex-col gap-gutter">
				{rows?.map((row, index) => {
					if (index === 0) {
						console.log(row.item[0])
					}
					return (
						<div key={row._key} className="-mx-half-gutter flex">
							{row?.item?.map((mediaItem, index) => {
								let aspect1 = row?.item[0]?.aspectRatio
								let aspect2 = row?.item[1]?.aspectRatio
								let aspect3 = row?.item[2]?.aspectRatio
								...
								return (
									<div
										...
										style={{
											boxSizing: 'border-box', flex: widths[index],
